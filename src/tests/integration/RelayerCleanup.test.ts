...
describe('Relayer Cleanup', () => {
  ...
  it('should replace the pending transaction with lowest nonce using an approve call', async () => {
    ...
    try {
      ...
      const tx = pending.reduce(
        (lowest: RelayerTransaction | null, current: RelayerTransaction) =>
  ...
  it('lists and cleans up pending transactions on the relayer', async () => {
    ...
    try {
      ...
      const status =
        (await client.relaySigner.getRelayerStatus()) as RelayerStatus;
      logger.info('Initial relayer status:', { ...status } as Record<
        string,
        unknown
      >);
      ...
      const pendingTransactionsResponse =
        await client.relaySigner.listTransactions({
      ...
      const csvHeader =
        'nonce,hash,status,transactionId,to,value,data,speed,gasLimit,type,gasDetails,sentAt';
      ...
      logger.info('Pending transactions written to CSV file');
      logger.debug('CSV content:', { content: csvContent });
      ...
      logger.info('Starting to replace pending transactions');

      for (const tx of pendingTransactions as RelayerTransaction[]) {
        if (tx.status !== 'pending') {
          logger.info('Skipping non-pending transaction', {
            transactionId: tx.transactionId,
          });
          continue;
        }
        logger.info('Replacing transaction', {
          transactionId: tx.transactionId,
          nonce: tx.nonce,
        });
      ...
      logger.info('Final pending transactions:', {
        count: finalPendingTransactions.length,
      });

      // We expect no pending transactions to remain
      const remainingPending = (
        finalPendingTransactions as RelayerTransaction[]
