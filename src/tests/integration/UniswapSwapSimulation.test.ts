...
describe('Uniswap Swap Simulation', () => {
  ...
  describe('OBOL to ETH swap simulation', () => {
    it('should simulate the complete swap flow including approval and swap', async () => {
      ...
      const approvalSimulation = await simulationService.simulateTransaction(approvalTx, {
        networkId: '1',
        overrides: {
          [TEST_WALLET]: {
      ...
      const swapCalldata = routerInterface.encodeFunctionData('swapExactTokensForETH', [
        SWAP_AMOUNT,
      ...
      const swapSimulation = await simulationService.simulateTransaction(swapTx, {
        networkId: '1',
        overrides: {
          [TEST_WALLET]: {
            balance: ethers.parseEther('1').toString(), // Ensure enough ETH for gas
            state: {
              ...
              [`0x${ethers.zeroPadValue(TEST_WALLET, 32).slice(2)}`]: ethers.toBeHex(SWAP_AMOUNT)
            }
          }
        },
      ...
      console.log('Gas cost estimation:', {
        ...
        gasPriceDetails: {
          baseFeePerGas: gasCostEstimate.gasPriceDetails?.baseFeePerGas + ' gwei',
