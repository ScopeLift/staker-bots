...
import { ISwapStrategy, SwapConfig, SwapQuote } from '../interfaces/ISwapStrategy'
...
export class UniswapSwapStrategy implements ISwapStrategy {
  ...
  async getSwapQuote(amountIn: bigint): Promise<SwapQuote> {
    ...
    // Cap amount to max
    const actualAmountIn = amountIn > this.config.maxAmountIn 
      ? this.config.maxAmountIn 
      : amountIn
    ...
    // Get expected output
    const amounts = await this.routerContract.getAmountsOut(actualAmountIn, path)
    if (!amounts || amounts.length < 2 || !amounts[1]) {
      throw new Error('Invalid amounts returned from Uniswap')
    }
    
    // Calculate minimum output with slippage tolerance
    const amountOutMin = amounts[1] * BigInt(Math.floor((100 - this.config.slippageTolerance) * 100)) / BigInt(10000)
