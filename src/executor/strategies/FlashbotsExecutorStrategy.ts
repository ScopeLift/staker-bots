...
export class FlashbotsExecutorStrategy {
  ...
  public async submitTransaction(
    ...
    if (!isInitialized || !this.flashbotsProvider) {
      this.logger.warn('Flashbots provider not initialized, skipping MEV protection')
      return null
    }
      ...
      if (txHash) {
        ...
        while (!confirmed && attempts < maxAttempts) {
          ...
          if (attempts >= maxAttempts) {
            this.logger.warn('Max attempts reached waiting for transaction confirmation', {
              txHash,
              attempts
            })
          }
        ...
        this.logger.warn('Failed to submit transaction via Flashbots - no txHash returned')
        return null
      }
    } catch (error) {
