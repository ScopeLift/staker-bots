...
export class RelayerExecutorWithMEV extends RelayerExecutor {
  ...
  constructor(
    ...
    if (this.useMevProtection) {
      if (config.wallet?.privateKey) {
        ...
        this.logger.info(
          'MEV protection initialized with Flashbots and private wallet',
        );
      } else {
        this.logger.warn(
  ...
  protected async executeTransaction(tx: QueuedTransaction): Promise<void> {
    ...
    if (
      !this.useMevProtection ||
      !this.flashbotsStrategy ||
      !this.privateWallet
    ) {
    ...
    try {
      ...
      const gasLimit =
        tx.profitability.estimates.gas_estimate || BigInt(300000);
      ...
      const txHash =
        await this.flashbotsStrategy.submitTransaction(transaction);
      ...
      if (txHash) {
        ...
        try {
          ...
          if (receipt && receipt.status === 1) {
            ...
            if (this.db) {
              try {
                ...
                this.logger.error(
                  'Failed to clean up queue items after successful transaction',
                  {
                    error:
          ...
          tx.error =
            waitError instanceof Error
              ? waitError
              : new Error('Failed to confirm transaction');
        }
      }

      // If we get here, Flashbots submission failed or transaction failed
      this.logger.info(
        'Flashbots submission unsuccessful, falling back to standard execution',
      );
    } catch (error) {
  ...
  private async cleanupQueueItems(
    ...
    const parentExecutor = this as unknown as {
      cleanupQueueItems?: (
        tx: QueuedTransaction,
        txHash: string,
      ) => Promise<void>;
    };
    if (typeof parentExecutor.cleanupQueueItems === 'function') {
      await parentExecutor.cleanupQueueItems(tx, txHash);
    } else {
      ...
      if (depositIds && depositIds.length > 0) {
        for (const depositId of depositIds) {
          ...
          try {
            ...
            const queueItem =
              await this.db.getTransactionQueueItemByDepositId(depositIdStr);
            if (queueItem) {
              await this.db.deleteTransactionQueueItem(queueItem.id);
              this.logger.info(
                `Cleaned up queue item for deposit ${depositIdStr}`,
              );
            }
          } catch (error) {
            this.logger.warn(
              `Failed to clean up queue item for deposit ${depositIdStr}`,
              {
                error: error instanceof Error ? error.message : String(error),
              },
            );
