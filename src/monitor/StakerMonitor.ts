...
export class StakerMonitor extends EventEmitter {
  ...
  private async processBlockRange(
    ...
    addEventsToGroup(sortedEvents.stakedWithAttribution, 'stakedWithAttribution')
    ...
    const txEntries = [...eventsByTx.entries()]
      .map(([txHash, events]) => ({
        ...
        blockNumber: events.deposited?.blockNumber ||
                    ...
                    events.depositUpdated?.blockNumber || 0,
      }))
  ...
  private async processTransactions(txEntries: TransactionEntry[]): Promise<void> {
    for (const { events } of txEntries) {
      ...
      if (events.deposited) {
        ...
        let delegateeAddress = events.altered?.args.newDelegatee || DEFAULT_DELEGATEE_ADDRESS
    ...
    await this.withRetry(async () => {
      try {
        ...
        if (result.success || !result.retryable) {
          await this.updateGovLstDeposit(event.depositId, event.delegateeAddress)
          return
        }
    ...
    while (attempts < this.config.maxRetries) {
      try {
        ...
        if (attempts === this.config.maxRetries) {
          this.logger.error(`Failed to ${context} after max retries`, { error })
          throw error
        }
